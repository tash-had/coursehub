...
export class CourseReviewFormComponent implements OnInit {
  ...
  submitReview() {
    ...
    // NEED TO GET PARENTID
    
    let usefulnessRating: number = 5 * (this.usefulnessRatingSlider.ratingSlider.nativeElement.value/100);
    let difficultyRating: number = 5 * (this.difficultyRatingSlider.ratingSlider.nativeElement.value/100);
    let optionalComment: string = this.optionalCommentInput.inputText;
        
    let usefulnessRatingStr = "<br><div class='inCommentRatingsContainer'><span class='inCommentRating'>" + usefulnessRating +"</span><br>";
    let difficultyRatingStr = "<span class='inCommentRating'>" + difficultyRating +"</span></div>";
    ...
    this.courseReviewService.reviewCourse(difficultyRating, usefulnessRating, this.courseData['id_'], localStorage.getItem("user_id")).subscribe(() => {});
